...
"""Shared utilities for wgrad and dgrad dumping."""
...
class WGradBuffer:
    """Buffer for collecting wgrads before batch save."""
    ...
    def add(self, layer_name: str, grad: torch.Tensor, iteration: int, save_dir: str):
        if self._iteration is None:
            self._iteration = iteration
            self._save_dir = save_dir

        self._buffer[layer_name] = grad.detach().cpu()

    def flush(self):
        if not self._buffer or not self._save_dir or self._iteration is None:
            self.reset()
            return

        rank_info = get_rank_info()
        if should_save_edp(rank_info):
            logger.info(
                f"  [{datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')}] saving wgrads "
                f"from iteration {self._iteration:7d}"
            )

            iter_dir = get_wgrad_iter_dir(self._save_dir, self._iteration)
            checkpoint_name = get_wgrad_checkpoint_name(rank_info)
            filepath = os.path.join(iter_dir, f"{checkpoint_name}.pth")

            save_dict = {"model_chunk0": dict(self._buffer)}
            torch.save(save_dict, filepath)

            num_params = len(self._buffer)
            total_elements = sum(t.numel() for t in self._buffer.values())
            logger.info(
                f"  [{datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')}] saved wgrads "
                f"from iteration {self._iteration:7d} "
                f"({num_params} params, {total_elements:,} elements) to {filepath}"
            )
